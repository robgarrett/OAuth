...
[CmdletBinding()]Param(
	[Parameter(Mandatory = $false)][string]$SubjectName = "CN=exodemo@ssa.gov",
	[Parameter(Mandatory = $false)][string]$FileNameNoExt = "EXODemo"
)
...
$signedCertParams = @{
	...
	'FriendlyName' = 'EXO Demo OAUTH Certificate'
...
# Clean out old certs.
Get-item Cert:\LocalMachine\my\* | ? { $_.Subject -ieq $SubjectName } | Remove-Item;

# Create a PFX
New-SelfSignedCertificateEx @signedCertParams;
$pfxCert = Get-Item Cert:\LocalMachine\my\* | ? { $_.Subject -ieq $SubjectName };
$certBytes = $pfxCert.Export([Security.Cryptography.X509Certificates.X509ContentType]::Pfx, $SecurePassword);
[System.IO.File]::WriteAllBytes("$PSScriptRoot\$($FileNameNoExt).pfx", $certBytes);

# Generate .CER
$certBytes = $pfxCert.Export([Security.Cryptography.X509Certificates.X509ContentType]::Cert);
[System.IO.File]::WriteAllBytes("$PSScriptRoot\$($FileNameNoExt).cer", $certBytes);
